...
def describe_anomalie_adresse_service():
    def describe_init():
        ...
        def it_raise_error(mocker, session):
            with pytest.raises(TypeError):
                service = AnomalieAdresseService()
    ...
    def describe_create_anomalie():
        ...
        def it_create_anomalie(anomalie_adresse_service):
            anomalie_model = AnomalieAdresse(status='status', priority='priority', code_oi='code_oi')
            anomalie_expect = AnomalieAdresse(status=Status.ACKNOWLEDGED, priority='priority', code_oi='code_oi')

            anomalie_adresse_service.create_anomalie(anomalie_model)
            
            anomalie_adresse_service.anomalie_adresse_manager.create_anomalieAdresse.assert_called_once_with(anomalie_expect)
    ...
    def describe_read_anomalie():
        def it_return_anomalie(anomalie_adresse_service):
            ...
            assert anomalie_adresse_service.read_one_anomalie("test") == "result"
    ...
    def describe_cancel_anomalie():
        def it_cancel_anomalie(anomalie_adresse_service):
            ...
            anomalie_adresse_service.anomalie_adresse_manager.cancel_anomalie.assert_called_once_with("test")